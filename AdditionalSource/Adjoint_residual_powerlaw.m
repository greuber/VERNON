function [ drdx ] = Adjoint_residual_powerlaw( NUM,PAR,MESH,h )
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

MESH.CompVar.powerlaw_ini = MESH.CompVar.powerlaw;
for id1 = 1:2
    NUM.Solve.f_res_brutal = zeros(NUM.no_nodes*(NUM.ndof-1)+NUM.no_nodes_linear,1);
    if id1 == 1
        MESH.CompVar.powerlaw(unique(NUM.O_ind)) = MESH.CompVar.powerlaw(unique(NUM.O_ind))+h;
    else
        MESH.CompVar.powerlaw(unique(NUM.O_ind)) = MESH.CompVar.powerlaw_ini(unique(NUM.O_ind));
    end
