function [ NUM ] = get_element_res_gravity( p,u,x,PAR ,MESH,NUM,i)
...
NUM.f_line_val   = zeros(4,1);
   ...
   NUM.mu = MESH.CompVar.mu_ref(NUM.Number.number_quad(j,i)).*(str_invariant./MESH.CompVar.str_ref(NUM.Number.number_quad(j,i))).^(1./MESH.CompVar.powerlaw(NUM.Number.number_quad(j,i)) - 1);
   NUM.mu_2d(NUM.Number.number_quad(j,i)) = NUM.mu;
        ...
        NUM.f_quad_val = NUM.f_quad_val - (NUM.Solve.N_matrix'*MESH.CompVar.rho(NUM.Number.number_quad(9,i))'* [1;1] * MESH.INTP.weight(j) * NUM.Solve.detJ) ;
