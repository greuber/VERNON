...
if random == 1
    x = x + 0.2*[rand(size(x))-0.5]*NUM.NUMERICS.dx;             % Add random distribution additionally for variable spacing
    z = z + 0.2*[rand(size(z))-0.5]*NUM.NUMERICS.dz;             % Add random distribution additionally for variable spacing
end
...
switch PAR.Perturb
    ...
    case 'middle'
        ...
        % make sinus only in the middle of the model
        st                              = PAR.A0/sin(pi/2);
        k_topo          = linspace((0),(2*pi),NUM.NUMERICS.no_nodes_x);
        z_topo          = [sin(k_topo)];
        z_topo          = st.*z_topo;
    ...
    case 'random'
        ...
        ind = find(MESH.GCOORD(2,:)<3e3+bounding & MESH.GCOORD(2,:)>3e3-bounding);
        MESH.GCOORD(2,ind) = MESH.GCOORD(2,ind) + 80*[rand(size(MESH.GCOORD(2,ind)))];
