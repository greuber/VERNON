function [ beta_low,beta_step_star,beta_high,grad,fcost,dcost_du ] = zoom_powerlaw( beta_low,beta_high,NUM,MESH,PAR,dcost_du_old,dcost_du,fcost,fcost_old,beta_step,beta_step_old,sol_ini,dx,ind_X,fcost_ini,grad_ini)
%UNTITLED3 Summary of this function goes here
%   Detailed explanation goes here
k = 1;  % couning the trial steps
...
while k<10
    ...
    MESH.CompVar.powerlaw(unique(NUM.O_ind_powerlaw)) = sol_ini(unique(NUM.O_ind_powerlaw)) + beta_low*dx;
    ...
    MESH.CompVar.powerlaw(unique(NUM.O_ind_powerlaw)) = sol_ini(unique(NUM.O_ind_powerlaw)) + beta_step*dx;
    [ NUM,MESH ] = Adjoint_StokesSolution( MESH,PAR,NUM );
    fcost = (1/2) * (NUM.r(ind_X) - NUM.r_ini(ind_X))' * (NUM.r(ind_X) - NUM.r_ini(ind_X));
    
    if (fcost-1e-13) >= fcost_ini + 1e-4 * beta_step * grad_ini' * dx || (fcost-1e-13) >= fcost_low       % minus -13 as equal is hard to get in matlab!
        beta_high = beta_step;
    else
        % Compute gradient
        dcost_du(ind_X) = (NUM.r(ind_X)' - NUM.r_ini(ind_X)');
        h_max = 1e-8;
        h = max([(1e-6*abs(MESH.CompVar.powerlaw)), h_max]);
        [ drdx ] = Adjoint_residual_powerlaw( NUM,PAR,MESH,h );
